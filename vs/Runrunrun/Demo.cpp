...
void Demo::DrawPlayerSprite() {
	...
	model = translate(model, vec3(xpos, ypos-53, 0.0f));
	// Scale sprite 
	model = scale(model, vec3(frame_width, frame_height, 1));
	glUniformMatrix4fv(glGetUniformLocation(this->program, "model"), 1, GL_FALSE, value_ptr(model));
...
void Demo::BuildPlayerSprite()
{
	...
	// Load, create texture 
	int width, height;
	unsigned char* image = SOIL_load_image("charautama.png", &width, &height, 0, SOIL_LOAD_RGBA);
...
void Demo::BuildHeartSprite()
{
	this->programHeart = BuildShader("heartSprite.vert", "heartSprite.frag");
	UseShader(this->programHeart);

	// Load and create a texture 
	glGenTextures(1, &textureHeart);
	glBindTexture(GL_TEXTURE_2D, textureHeart); // All upcoming GL_TEXTURE_2D operations now have effect on our texture object
	...
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, EBOHeart);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(indices), indices, GL_STATIC_DRAW);
...
void Demo::DrawHeartSprite(int jarak) {
	...
	// Activate shader
	UseShader(this->programHeart);
	glUniform1i(glGetUniformLocation(this->programHeart, "ourTexture"), 1);
